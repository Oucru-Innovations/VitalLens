...
Packaging must also ship the **distribution metadata** of `paddlex[ocr-core]` (`opencv-contrib-python`, `shapely`, `pyclipper`, `python-bidi`, `imagesize`, `pypdfium2`, `pillow`). `paddlex/utils/deps.py` guards `import cv2` and friends behind `is_dep_available(<dist name>)`, which resolves through `importlib.metadata.version()`; the names are runtime strings, so neither Nuitka's auto-detection nor `collect_all()` (which follows *import* names — `cv2`, `PIL`) covers them. Without the dist-info the import is silently skipped and the run dies mid-pipeline with `NameError: name 'cv2' is not defined`. `_check_paddlex_deps()` in `processing/xray.py` re-checks the same list before building the OCR instance so a future packaging regression names its own cause.
